...
def check_output_folder(output_folder):
    ...
    if os.path.exists(quantification_file) and profile_file:
        ...
        raise OutputFolderIncompleteException('The folder %s  is not a valid analysis.' % output_folder)
...
def main():
    ...
    try:
        ...
        get_name_from_folder=lambda x: os.path.basename(os.path.abspath(x)).replace('RESULTS_on_','')
        ...
        OUTPUT_DIRECTORY='Compare_on_%s' % database_id

        if args.output_folder:
            OUTPUT_DIRECTORY=os.path.join(os.path.abspath(args.output_folder),OUTPUT_DIRECTORY)

        _jp=lambda filename: os.path.join(OUTPUT_DIRECTORY,filename) #handy function to put a file in the output directory
        log_filename=_jp('Compare_RUNNING_LOG.txt')
