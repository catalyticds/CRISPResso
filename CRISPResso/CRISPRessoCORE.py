...
# Added by Catalytic
# This method will shorten overly long sgRNA sequences before crispresso inserts them into filenames.
# The max filesize for linux is 255 characters.
# The longest filename currently using the sgRNA is the Alleles_frequency_table_around_cut_site_for_<sgRNA>.txt
# which has 48 extra characters.
def shorten_sgrna(sgRNA):
    return sgRNA[:min(255-48, len(sgRNA))]
...
offsetparam_overhang = []
def get_row_around_cut(row,cut_point,offset):
    cut_idx=row['ref_positions'].index(cut_point)
    if (cut_idx-offset+1 < 0) and (cut_idx-offset+1 not in offsetparam_overhang):
        offsetparam_overhang.append(cut_idx-offset+1)
...
def plot_alleles_table(reference_seq,cut_point,df_alleles,sgRNA_name,OUTPUT_DIRECTORY,MIN_FREQUENCY=0.5,MAX_N_ROWS=100):
    ...
    # Catalytic
    # If this var is set dynamically as above it will cause a mislabeling of the cut site when the var value extends beyond the read
    ...
    if offset_around_cut_to_plot == 0:
        if (args.offset_around_cut_to_plot - (cut_point+1))*-1 > min(offsetparam_overhang):
            warn('A reference/amplicon string was not found in the first line of df_alleles. The offset parameter needs to be reduced due to overhang off of at least one amplicon.')
            raise Exception('Please try reducing the "Offset around cut site (bp)" parameter from {} to {}.'.format(args.offset_around_cut_to_plot, args.offset_around_cut_to_plot + min(offsetparam_overhang)))
        else:
            warn('A reference/amplicon string was not found in the first line of df_alleles. The offset parameter needs to be reduced due to overhang off of the reference.')
            raise Exception('Please try reducing the "Offset around cut site (bp)" parameter from {} to {}.'.format(args.offset_around_cut_to_plot, args.offset_around_cut_to_plot - (args.offset_around_cut_to_plot - (cut_point+1))))
    if cut_point+1-offset_around_cut_to_plot < 0:
        warn('The offset parameter needs to be reduced due to overhang off of the reference.')
        raise Exception('Please try reducing the "Offset around cut site (bp)" parameter from {} to {}.'.format(args.offset_around_cut_to_plot, args.offset_around_cut_to_plot + (cut_point+1-offset_around_cut_to_plot)))
    ...
    #Catalytic Data Science add z_cut option/param
    if args.z_cut > 0:
        ax_hm.vlines([offset_around_cut_to_plot-2],*ax_hm.get_ylim(),linestyles='dashed')
        ax_hm.vlines([offset_around_cut_to_plot+2],*ax_hm.get_ylim(),linestyles='dashed')
        ax_hm_ref.vlines([offset_around_cut_to_plot-2],*ax_hm.get_ylim(),linestyles='dashed')
        ax_hm_ref.vlines([offset_around_cut_to_plot+2],*ax_hm.get_ylim(),linestyles='dashed')
    else:
    ...
    ax_hm_ref.legend(proxies, descriptions, numpoints=1, markerscale=2, loc='center', bbox_to_anchor=(0.5, 4.5),ncol=1)

    _jp=lambda filename: os.path.join(OUTPUT_DIRECTORY,filename)

    plt.savefig(_jp('9.Alleles_around_cut_site_for_%s.pdf' % shorten_sgrna(sgRNA_name)),bbox_inches='tight')
    if args.save_also_png:
        plt.savefig(_jp('9.Alleles_around_cut_site_for_%s.png' % shorten_sgrna(sgRNA_name)),bbox_inches='tight',pad=1)


def main():
    # catalytic
    error_log_filename=('CRISPResso_PARAM_LOG.txt')
    logging.getLogger().addHandler(logging.FileHandler(error_log_filename))

    try:
        print '  \n~~~CRISPResso~~~'
        print '-Analysis of outcomes from deep sequencing data-'
        print'''
                      )
                     (
                    __)__
        ...
        parser.add_argument('--hide_mutations_outside_window_NHEJ',help='This parameter allows to visualize only the mutations overlapping the cleavage site and used to classify a read as NHEJ. This parameter has no effect on the quantification of the NHEJ. It  may be helpful to mask a pre-existing and known mutations or sequencing errors outside the window used for quantification of NHEJ events.',action='store_true')
        ...
        parser.add_argument('--z_cut', type=int, help='Represent z-cut/sticky ends in plots. Cut location in bp around cut site to plot. Two cut sites will not be represented and original cut site will not be displayed. Added by catalytic data science.', default=0)
        parser.add_argument('--cleavage_offset', type=int, help="Cleavage offset to use within respect to the 3' end of the provided target site sequence. Remember that the target site sequence must be entered without the PAM. The default is -3 and is suitable for the SpCas9 system. For alternate nucleases, other cleavage offsets may be appropriate, for example, if using Cpf1 this parameter would be set to 1.", default=-3)
        ...
        if wrong_nt:
            raise NTException('Some characters in the amplicon sequence are incorrect: %s' % ' '.join(wrong_nt))
        ...
        if args.guide_seq:
            ...
            for current_guide_seq in args.guide_seq.split(','):
                ...
                if wrong_nt:
                    raise NTException('Some characters in the target site sequence are incorrect: %s'  % ' '.join(wrong_nt))
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            if wrong_nt:
                raise NTException('Some characters in the amplicon sequence expected after an HDR are incorrect: %s' % ' '.join(wrong_nt))
        ...
        if args.donor_seq:
            ...
            if wrong_nt:
                raise NTException('Some characters in the donor sequence are incorrect: %s' % ' '.join(wrong_nt))
        ...
        if args.coding_seq:
            ...
            for exon_seq in args.coding_seq.strip().upper().split(','):
                ...
                if wrong_nt:
                    raise NTException('Some characters in the coding sequence are incorrect: %s' % ' '.join(wrong_nt))
        ...
        OUTPUT_DIRECTORY='RESULTS_on_%s' % database_id

        if args.output_folder:
            OUTPUT_DIRECTORY=os.path.join(os.path.abspath(args.output_folder),OUTPUT_DIRECTORY)

        _jp=lambda filename: os.path.join(OUTPUT_DIRECTORY,filename) #handy function to put a file in the output directory
        log_filename=_jp('RUNNING_LOG.txt')
        ...
        try:
            ...
            with open(log_filename,'w+') as outfile:
                outfile.write('[Parameters used]:\n %s\n\n[Execution log]:\n' % ' '.join(sys.argv).replace('CRISPResso', ''))
        ...
        cmd=(('cat %s |'% processed_output_filename )+ \
            ...
            " | sed 's/:/_/g' | needle -supper1 -asequence=%s -supper2 -bsequence=/dev/stdin -outfile=/dev/stdout %s 2>> %s  | gzip >%s" \
        ...
        if sr_not_aligned.count():
            ...
            # Catalytic
            with gzip.open(fasta_not_aligned_filename, 'w+') as outfile:
                for x in sr_not_aligned.iteritems():
                    outfile.write('>%s\n%s\n' % (x[0],reverse_complement(x[1])))
            ...
            # catalytic
            cmd="zcat < %s | sed 's/:/_/g' | needle -supper1 -asequence=%s -supper2 -bsequence=/dev/stdin -outfile=/dev/stdout %s 2>> %s  | gzip >%s" \
                %(fasta_not_aligned_filename,database_fasta_filename,args.needle_options_string,log_filename,needle_output_rc_filename)
            ...
            if args.expected_hdr_amplicon_seq:
                # catalytic
                cmd="zcat < %s | sed 's/:/_/g' | needle -supper1 -asequence=%s -supper2 -bsequence=/dev/stdin -outfile=/dev/stdout %s 2>> %s  | gzip >%s" \
                    %(fasta_not_aligned_filename,database_repair_fasta_filename,args.needle_options_string,log_filename,needle_output_repair_rc_filename)
            ...
            # Catalytic
            df_needle_alignment_rc['ref_seq']=df_needle_alignment_rc['ref_seq'].str.upper()
            df_needle_alignment_rc['align_seq']=df_needle_alignment_rc['align_seq'].str.upper()
        ...
        if args.n_processes > 1:
            info('[Quantifications are running in parallel mode with %d processes]' % min(df_needle_alignment.shape[0],args.n_processes) )
        ...
        font_dirs = ['/usr/local/lib/python2.7/dist-packages/matplotlib/mpl-data/fonts/Open_Sans/static/OpenSans/', ]
        ...
        plt.rcParams['axes.labelcolor'] = cds_gray
        ...
        # info(plt.rcParams) #prints all available rcParams

        fig, ax = plt.subplots(figsize=(10.5,10))
        ax.set_color_cycle([cds_orange, cds_blue, cds_green, cds_magenta])
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
        ax.spines['bottom'].set_color(cds_blue)
        ax.spines['bottom'].set_linewidth(2)
        ax.spines['left'].set_color(cds_blue)
        ...
        plt.bar(0,hdensity[center_index],linewidth=0)
        barlist=plt.bar(hlengths,hdensity,align='center',linewidth=0)
        barlist[center_index].set_color(cds_orange)
        plt.xlim([xmin,xmax])
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.ylabel('Sequences (no.)', color=cds_gray, fontweight='light')
        plt.xlabel('Indel size (bp)', color=cds_gray, fontweight='light')
        plt.ylim([0,hdensity.max()*1.2])
        plt.title('Indel size distribution', color=cds_black, fontweight='bold')
        plt.legend(['No indel','Indel'], ncol=2, frameon=False, loc='center', bbox_to_anchor=(0.5, -0.25))
        ...
        fig, ax = plt.subplots(figsize=(10.5,10))
        ax.set_color_cycle([cds_orange, cds_blue])
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
        ax.spines['bottom'].set_color(cds_blue)
        ax.spines['bottom'].set_linewidth(2)
        ...
        ax.spines['left'].set_linewidth(2)
        ax.margins(0.2)
        plt.bar(0,hdensity[center_index]/(float(hdensity.sum()))*100.0,linewidth=0)
        ...
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Indel size distribution', color=cds_black, fontweight='bold')
        plt.ylabel('Sequences (%)', color=cds_gray, weight='light')
        plt.xlabel('Indel size (bp)', color=cds_gray, fontweight='light')
        plt.legend(['No indel','Indel'], ncol=2, frameon=False, loc='center', bbox_to_anchor=(0.5, -0.25))
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            patches, texts, autotexts =ax1.pie([N_UNMODIFIED,N_MIXED_HDR_NHEJ,N_MODIFIED,N_REPAIRED], \
                                               ...
                                               explode=(0,0,0,0), \
                                               colors=[cds_orange, cds_blue, cds_magenta, cds_green],autopct='%1.1f%%',
                                               wedgeprops=dict(edgecolor='w', linewidth=10))
            ...
            if cut_points or args.donor_seq:
                ...
                if cut_points:
                    ax2.plot(cut_points+offset_plots,np.zeros(len(cut_points)),'vr',color=cds_orange, ms=24,label='Predicted cleavage site/s')

                for idx,sgRNA_int in enumerate(sgRNA_intervals):
                    if idx==0:
                        ax2.plot([sgRNA_int[0],sgRNA_int[1]],[0,0],lw=10,c=(0,0,0,0.15),label='Target Site')
                    else:
            ...
            plt.savefig(_jp('2.Unmodified_NHEJ_HDR_pie_chart.pdf'),pad_inches=1)
            if args.save_also_png:
                plt.savefig(_jp('2.Unmodified_NHEJ_HDR_pie_chart.png'),pad_inches=1)

        else:
            fig=plt.figure(figsize=(12*1.5,14.5*1.5))
            ax1 = plt.subplot2grid((6,3), (0, 0), colspan=3, rowspan=5)
            patches, texts, autotexts =ax1.pie([N_UNMODIFIED/N_TOTAL*100,N_MODIFIED/N_TOTAL*100], \
                                               labels=[r'$\mathbf{Unmodified}$' '\n(%d reads)' %N_UNMODIFIED, \
                                                       r'$\mathbf{NHEJ}$' '\n(%d reads)' % N_MODIFIED], \
                                               explode=(0,0),colors=[cds_orange, cds_blue],autopct='%1.1f%%',
                                               wedgeprops=dict(edgecolor='w', linewidth=10),labeldistance=1.2)
            ...
            if cut_points:
                ...
                for idx,sgRNA_int in enumerate(sgRNA_intervals):
                    if idx==0:
                        ax2.plot([sgRNA_int[0],sgRNA_int[1]],[0,0],lw=10,c=(0,0,0,0.15),label='Target Site',solid_capstyle='butt')
                    else:
                        ax2.plot([sgRNA_int[0],sgRNA_int[1]],[0,0],lw=10,c=(0,0,0,0.15),label='_nolegend_',solid_capstyle='butt')

                ax2.plot(cut_points+offset_plots,np.zeros(len(cut_points)),'vr', color=cds_orange, ms=12,label='Predicted cleavage site/s')
        ...
        ax.spines['left'].set_color(cds_blue)
        ax.spines['left'].set_linewidth(2)
        ax.spines['top'].set_color(cds_blue)
        ax.spines['top'].set_linewidth(2)
        ax.spines['right'].set_color(cds_blue)
        ax.spines['right'].set_linewidth(2)


        ax.bar(x_bins_ins[:-1],y_values_ins,align='center',linewidth=0)
        barlist=ax.bar(x_bins_ins[:-1],y_values_ins,align='center',linewidth=0,color=cds_blue)
        barlist[0].set_color(cds_orange)
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Insertions', color=cds_black, fontweight='bold')
        plt.xlabel('Size (bp)', color=cds_gray, fontweight='light', labelpad=8)
        plt.ylabel('Sequences % (no.)', color=cds_gray, fontweight='light')
        plt.legend(['Non-insertion','Insertion'][::-1], frameon=False, loc='center', bbox_to_anchor=(.5, -.38), ncol=2)
        plt.xlim(xmin=-1)
        ...
        barlist=ax.bar(-x_bins_del[:-1],y_values_del,align='center',linewidth=0,color=cds_blue)
        barlist[0].set_color(cds_orange)
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Deletions', color=cds_black, fontweight='bold')
        plt.xlabel('Size (bp)', color=cds_gray, fontweight='light', labelpad=8)
        plt.ylabel('Sequences % (no.)', color=cds_gray, fontweight='light')
        plt.legend(['Non-deletion','Deletion'][::-1], bbox_to_anchor=(.5, -.38), ncol=2, frameon=False, loc='center')
        plt.xlim(xmax=1)
        y_label_values= np.round(np.linspace(0, min(N_TOTAL,max(ax.get_yticks())),6))# np.arange(0,y_max,y_max/6.0)
        plt.yticks(y_label_values,['%.1f%% (%d)' % (n_reads/N_TOTAL*100,n_reads) for n_reads in y_label_values])
        ...
        barlist=ax.bar(x_bins_mut[:-1],y_values_mut,align='center',linewidth=0,color=cds_blue)
        barlist[0].set_color(cds_orange)
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Substitutions', color=cds_black, fontweight='bold')
        plt.xlabel('Positions substituted (number)', color=cds_gray, fontweight='light', labelpad=8)
        plt.ylabel('Sequences % (no.)', color=cds_gray, fontweight='light')
        # plt.setp(ax.get_xlabel(), padding=10)
        plt.legend(['Non-substitution','Substitution'][::-1], bbox_to_anchor=(.5, -.38), ncol=2, frameon=False, loc='center')
        plt.xlim(xmin=-1)
        y_label_values= np.round(np.linspace(0, min(N_TOTAL,max(ax.get_yticks())),6))# np.arange(0,y_max,y_max/6.0)
        plt.yticks(y_label_values,['%.1f%% (%d)' % (n_reads/N_TOTAL*100,n_reads) for n_reads in y_label_values])
        ...
        plt.plot(effect_vector_any,color=cds_orange,lw=3,label='Combined Insertions/Deletions/Substitutions')
        #plt.hold(True)
        ...
        if cut_points:
            ...
            for idx,sgRNA_int in enumerate(sgRNA_intervals):
                if idx==0:
                    plt.plot([sgRNA_int[0],sgRNA_int[1]],[0,0],lw=10,c=(0,0,0,0.15),label='Target Site',solid_capstyle='butt')
                else:
                    plt.plot([sgRNA_int[0],sgRNA_int[1]],[0,0],lw=10,c=(0,0,0,0.15),label='_nolegend_',solid_capstyle='butt')


        lgd=plt.legend(loc='center', bbox_to_anchor=(0.5, -0.31),ncol=1,frameon=False)
        ...
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Mutation position distribution', color=cds_black, fontweight='bold')
        plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
        plt.ylabel('Sequences % (no.)', color=cds_gray, fontweight='light')
        ...
        ax.spines['right'].set_color(cds_blue)
        ax.spines['right'].set_linewidth(2)
        plt.plot(effect_vector_insertion,cds_orange,lw=3,label='Insertions')
        #plt.hold(True)
        plt.plot(effect_vector_deletion,cds_blue,lw=3,label='Deletions')
        plt.plot(effect_vector_mutation,cds_green,lw=3,label='Substitutions')
        ...
        lgd=plt.legend(loc='center', bbox_to_anchor=(0.5, -0.38),ncol=1,frameon=False)
        ...
        plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
        plt.ylabel('Sequences: % Total ( % NHEJ, no. )', color=cds_gray, fontweight='light')
        plt.ylim(0,max(1,y_max))
        plt.xlim(xmax=len(args.amplicon_seq)-1)
        plt.setp(ax.get_yticklabels(), fontweight="semibold")
        plt.setp(ax.get_xticklabels(), fontweight="semibold")
        plt.title('Mutation position distribution of NHEJ', color=cds_black, fontweight='bold')
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            ax.spines['right'].set_color(cds_blue)
            ax.spines['right'].set_linewidth(2)
            plt.plot(effect_vector_insertion_hdr,cds_orange,lw=3,label='Insertions')
            #plt.hold(True)
            plt.plot(effect_vector_deletion_hdr,cds_blue,lw=3,label='Deletions')
            plt.plot(effect_vector_mutation_hdr,cds_green,lw=3,label='Substitutions')
            ...
            lgd=plt.legend(loc='center', bbox_to_anchor=(0.5, -0.28),ncol=1, frameon=False)
            ylabel_values = np.arange(0,1,1.0/6.0)
            if y_max > 0:
            ...
            plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
            plt.ylabel('Sequences: % Total ( % HDR, no. )', color=cds_gray, fontweight='light')
            plt.ylim(0,max(1,y_max))
            plt.xlim(xmax=len(args.amplicon_seq)-1)
            plt.setp(ax.get_yticklabels(), fontweight="semibold")
            plt.setp(ax.get_xticklabels(), fontweight="semibold")
            plt.title('Mutation position distribution of HDR', color=cds_black, fontweight='bold')
            ...
            ax.spines['right'].set_color(cds_blue)
            ax.spines['right'].set_linewidth(2)
            plt.plot(effect_vector_insertion_mixed,cds_orange,lw=3,label='Insertions')
            #plt.hold(True)
            plt.plot(effect_vector_deletion_mixed,cds_blue,lw=3,label='Deletions')
            plt.plot(effect_vector_mutation_mixed,cds_green,lw=3,label='Substitutions')
            ...
            plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
            plt.ylabel('Sequences: % Total ( % mixed HDR-NHEJ, no. )',  color=cds_gray, fontweight='light')
            plt.ylim(0,max(1,y_max))
            plt.xlim(xmax=len(args.amplicon_seq)-1)
            plt.setp(ax.get_yticklabels(), fontweight="semibold")
            plt.setp(ax.get_xticklabels(), fontweight="semibold")
            plt.title('Mutation position distribution of mixed HDR-NHEJ', color=cds_black, fontweight='light')
        ...
        ax1.spines['bottom'].set_color(cds_blue)
        ax1.spines['bottom'].set_linewidth(2)
        ax1.spines['left'].set_color(cds_blue)
        ax1.spines['left'].set_linewidth(2)
        ax1.spines['top'].set_color(cds_blue)
        ax1.spines['top'].set_linewidth(2)
        ax1.spines['right'].set_color(cds_blue)
        ax1.spines['right'].set_linewidth(2)
        markerline, stemlines, baseline=ax1.stem(avg_vector_ins_all,cds_orange,lw=3,markerfmt="s",s=50)
        # plt.setp(baseline, 'linewidth', 0)
        plt.setp(ax1.get_yticklabels(), fontweight="semibold")
        plt.setp(ax1.get_xticklabels(), fontweight="semibold")
        plt.setp(stemlines, color=cds_orange, linewidth=2)
        plt.setp(markerline, markerfacecolor=cds_blue, markersize=3, fillstyle='full', markeredgecolor=cds_blue, zorder=10)
        ...
        plt.xticks(np.arange(0,len_amplicon,max(3,(len_amplicon/6) - (len_amplicon/6)%5)).astype(int) )
        plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
        plt.ylabel('Average insertion length', color=cds_gray, fontweight='light')
        plt.ylim(0,max(1,y_max))
        plt.xlim(xmax=len_amplicon-1)
        ax1.set_title('Position dependent insertion size', color=cds_black, fontweight='bold')
        plt.tight_layout()

        ax2=fig.add_subplot(1,2,2)
        ax2.spines['bottom'].set_color(cds_blue)
        ax2.spines['bottom'].set_linewidth(2)
        ax2.spines['left'].set_color(cds_blue)
        ax2.spines['left'].set_linewidth(2)
        ax2.spines['top'].set_color(cds_blue)
        ax2.spines['top'].set_linewidth(2)
        ax2.spines['right'].set_color(cds_blue)
        ax2.spines['right'].set_linewidth(2)
        markerline, stemlines, baseline=ax2.stem(avg_vector_del_all,cds_orange,lw=3,markerfmt="s",s=50)
        plt.setp(ax2.get_yticklabels(), fontweight="semibold")
        plt.setp(ax2.get_xticklabels(), fontweight="semibold")
        plt.setp(baseline, visible=False)
        plt.setp(stemlines, color=cds_blue, linewidth=2)
        plt.setp(markerline, markerfacecolor=cds_orange, markersize=3, fillstyle='full', markeredgecolor=cds_orange, zorder=10)
        ...
        plt.xlabel('Reference amplicon position (bp)', color=cds_gray, fontweight='light')
        plt.ylabel('Average deletion length', color=cds_gray, fontweight='light')

        plt.ylim(ymin=0,ymax=max(1,y_max))
        plt.xlim(xmax=len_amplicon-1)
        ax2.set_title('Position dependent deletion size', color=cds_black, fontweight='bold')
        ...
        if PERFORM_FRAMESHIFT_ANALYSIS:
            ...
            fig=plt.figure(figsize=(12*1.5,14.5*1.5))
            ax1 = plt.subplot2grid((6,3), (0, 0), colspan=3, rowspan=5)
            x = np.char.array([r'$\mathbf{Frameshift}\/\/\mathbf{mutation}$' '\n(%d reads)' %MODIFIED_FRAMESHIFT,
                               r'$\mathbf{In}\mathbf{-}\mathbf{frame}\/\/\mathbf{mutation}$' '\n(%d reads)' % MODIFIED_NON_FRAMESHIFT,
                               r'$\mathbf{Noncoding}\/\/\mathbf{mutation}$' '\n(%d reads)' %NON_MODIFIED_NON_FRAMESHIFT])
            y = np.array([MODIFIED_FRAMESHIFT, MODIFIED_NON_FRAMESHIFT, NON_MODIFIED_NON_FRAMESHIFT])
            percent = 100.*y/y.sum()

            patches, texts = ax1.pie(y, explode=(0.0,0.0,0.0), colors=[cds_blue, cds_green, cds_orange], wedgeprops=dict(edgecolor='w', linewidth=3))
            ...
            if cut_points:
                ax2.plot(cut_points+offset_plots,np.zeros(len(cut_points)),'vr', ms=25,label='Predicted cleavage site/s')
            ...
            frameshift_sum = float(sum(hist_frameshift.values()))
            y = (y / frameshift_sum * 100) if frameshift_sum > 0 else y
            ax1.bar(x-0.5,y,color=cds_blue)
            ...
            ax1.set_xticklabels([str(idx)  for idx in [idx for idx in range(-30,31) if (idx % 3==0)]],rotation='vertical')
            plt.title('Frameshift profile', fontweight='bold')
            ...
            inframe_sum = float(sum(hist_inframe.values()))
            y = (y / inframe_sum * 100) if inframe_sum > 0 else y
            ax2.bar(x-0.5,y,color=(cds_orange))
            ...
            plt.title('In-frame profile', fontweight='bold')
            plt.tight_layout()
            plt.ylabel('%')
            ...
            patches, texts, autotexts =ax.pie([SPLICING_SITES_MODIFIED, \
                                              ...
                                              labels=[r'$\mathbf{Potential}\/\/\mathbf{splice}\/\/\mathbf{sites}\/\/\mathbf{modified}$' '\n(%d reads)' %SPLICING_SITES_MODIFIED, \
                                                      r'$\mathbf{Unmodified}$' '\n(%d reads)' % (df_needle_alignment.shape[0]- SPLICING_SITES_MODIFIED)], \
                                              explode=(0.0,0), \
                                              colors=[cds_blue,cds_orange], \
                                              autopct='%1.1f%%')
            proptease = fm.FontProperties()
            proptease.set_size('xx-large')
            plt.setp(autotexts, fontproperties=proptease)
            ...
            ax.spines['right'].set_color(cds_blue)
            ax.spines['right'].set_linewidth(2)
            plt.plot(effect_vector_insertion_noncoding,'r',color=cds_orange,lw=3,label='Insertions')
            #plt.hold(True)
            plt.plot(effect_vector_deletion_noncoding,'m',color=cds_blue,lw=3,label='Deletions')
            plt.plot(effect_vector_mutation_noncoding,'g',color=cds_green,lw=3,label='Substitutions')
            ...
            lgd=plt.legend(loc='center', bbox_to_anchor=(0.5, -0.38),ncol=1, fancybox=True, shadow=True)
            plt.xticks(np.arange(0,len_amplicon,max(3,(len_amplicon/6) - (len_amplicon/6)%5)).astype(int) )
            ...
            plt.ylim(0,max(1,y_max))
            plt.xlim(xmax=len(args.amplicon_seq)-1)
            plt.title('Noncoding mutation position distribution', color=cds_black, fontweight='bold')
        ...
        for sgRNA,cut_point in zip(sgRNA_sequences,cut_points):
            ...
            try:
                ...
                df_allele_around_cut.to_csv(_jp('Alleles_frequency_table_around_cut_site_for_%s.txt' % shorten_sgrna(sgRNA)),sep='\t',header=True)
                ...
                info('ERROR: get_dataframe_around_cut error. {}'.format(e))
                fig = plt.figure()
                fig.suptitle("Figure 9 could not be created.\n{}".format(str(e)), fontsize=14, fontweight='bold')
                plt.savefig(os.path.join(OUTPUT_DIRECTORY,'9.Alleles_around_cut_site_ERROR.pdf'),bbox_inches='tight',pad=1)
                if args.save_also_png:
                    plt.savefig(os.path.join(OUTPUT_DIRECTORY,'9.Alleles_around_cut_site_for_%s.png' % shorten_sgrna(sgRNA)),bbox_inches='tight',pad=1)
        ...
        if not args.keep_intermediate:
            ...
            if sr_not_aligned.count():
                files_to_remove+=[fasta_not_aligned_filename,needle_output_rc_filename]

                if args.expected_hdr_amplicon_seq:
                    files_to_remove+=[needle_output_repair_rc_filename]
        ...
        if PERFORM_FRAMESHIFT_ANALYSIS:
            ...
            #CDS Output frameshift analysis in tabular format
            pd.DataFrame(hist_inframe.items(), columns=['indel_size', 'reads']).to_csv(_jp('inframe_histogram.txt'), index=None, sep='\t')
            pd.DataFrame(hist_frameshift.items(), columns=['indel_size', 'reads']).to_csv(_jp('frameshift_histogram.txt'), index=None, sep='\t')
            ...
            with open(_jp('Quantification_of_editing_frequency.txt'),'w+') as outfile:
                outfile.write(
                    ...
                    +('Frameshift analysis:\n\t- Noncoding mutation:%d reads\n\t- In-frame mutation:%d reads\n\t- Frameshift mutation:%d reads' % (NON_MODIFIED_NON_FRAMESHIFT, MODIFIED_NON_FRAMESHIFT, MODIFIED_FRAMESHIFT)))
        else:
            with open(_jp('Quantification_of_editing_frequency.txt'), 'w+') as outfile:
                outfile.write(
                    ('Quantification of editing frequency:\n\t- Unmodified:%d reads\n' % N_UNMODIFIED) \
        ...
        if cut_points:
            cp.dump(sgRNA_intervals, open( _jp('target-sequence_intervals.pickle'), 'wb' ) )
        ...
        print_stacktrace_if_debug()
        error('Target site error, please check your input.\n\nERROR: %s' % e)
        sys.exit(2)
