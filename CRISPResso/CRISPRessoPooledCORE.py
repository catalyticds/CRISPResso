...
def main():
    try:
        ...
        if RUNNING_MODE=='ONLY_AMPLICONS':
            ...
            bam_filename_amplicons= _jp('AMPLICONS_ALIGNED.bam')
        ...
        for idx,row in df_final_data.iterrows():
            ...
            if RUNNING_MODE=='ONLY_AMPLICONS' or RUNNING_MODE=='AMPLICONS_AND_GENOME':
                folder_name='RESULTS_on_%s' % slugify(idx)
            else:
                folder_name='RESULTS_on_REGION_%s_%d_%d' %(row.chr_id,row.bpstart,row.bpend )
        ...
        if RUNNING_MODE != 'ONLY_GENOME':
            ...
            if tot_reads > 0 and tot_reads_aligned/tot_reads < 0.5:
                ...
                top_unaligned_filename=_jp('TOP_UNALIGNED.txt')
